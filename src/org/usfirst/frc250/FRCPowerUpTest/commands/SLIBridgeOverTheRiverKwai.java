 ...
 *
 */
public class SLIBridgeOverTheRiverKwai extends CommandGroup {

	public SLIBridgeOverTheRiverKwai() {
		// Add Commands here:
		...
		if (ownedScaleSide != Side.UNKNOWN) {
			if (ownedScaleSide == Side.LEFT && startingSide == Side.LEFT) {
				...
				addSequential(new IntakeReleaseFast());
				addSequential(new WaitCommand(2));
				addSequential(new IntakeStop());
			} else if (ownedScaleSide == Side.RIGHT && startingSide == Side.LEFT) {
				addSequential(new DriveDistance(218.5,218.5));
				addSequential(new TurnRight90());
				addSequential(new DriveDistance(241,241));
				addSequential(new TurnLeft90());
				addParallel(new SetArmTargetToAbsolutePosition("Arm Pos 5 Value"));
				addSequential(new DriveDistance(88,88));	
				addParallel(new SetWristTargetToAbsolutePosition(Robot.prefs2.getInt("Wrist Down Position")));
				addSequential(new TurnLeft90());
				addSequential(new DriveDistance(20,20));
				addSequential(new IntakeReleaseFast());
				addSequential(new WaitCommand(2));
				addSequential(new IntakeStop());
				...
				addSequential(new WaitCommand(2));
				addSequential(new IntakeStop());
			} else if (ownedScaleSide == Side.LEFT && startingSide == Side.RIGHT) {
				addSequential(new DriveDistance(218.5,218.5));
				addSequential(new TurnLeft90());
				addSequential(new DriveDistance(241,241));
				addSequential(new TurnRight90());
